...
# Orc1Cdc6 and replication origins within the genomic features of Trypanosoma cruzi. As the data  #
...
table <- read.csv2('contigencyTableFeatureSyncFeature.csv',row.names = 1)
table
...
chisq <- chisq.test(table, p= c(0.02972, 0.05780, 0.00331, 0.90783, 0.00018, 0.00097, 0.00019),simulate.p.value = TRUE,B = 1000)
chisq <- chisq.test(table, p= c(0.02972, 0.05780, 0.00331, 0.90783, 0.00018, 0.00097, 0.00019))
chisq
...
cramersV(table, p= c(0.02972, 0.05780, 0.00331, 0.90783, 0.00018, 0.00097, 0.00019))
